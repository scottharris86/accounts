...
//Initialize Objects on app load------------
		...
		RenderAll : function(){
			...
			$("tr").each(function(){
				for(var i=0; i<linkedAccounts.length; i++){
					if($(this).attr('data-id')==linkedAccounts[i]){
		...
		init : function(){
			...
			$('input[name=checkbox]').on('click', function(){
				...
				if(this.checked==true){
					$(this).closest('.account').addClass('success');
					selectedAccount.push(id);
				}
				else{
					for(var idx=0; idx<selectedAccount.length; idx++){
						if(id==selectedAccount[idx]){
							selectedAccount.splice(idx, 1);
							$(this).closest('.account').removeClass('success');
						}
					}
				}
		...
		onClickRemoveAccount : function(){
			for(var i=0; i<selectedAccount.length; i++){
				...
				for(var idx in linkedAccounts){
					if(selectedAccount[i]==linkedAccounts[idx]){
	...
	AddAccountsPage : {
		init : function(){
			$('a[name=modal-save]').on('click', {parent: this}, this.onClickAddAccount);
			this.$el=$('.account');

			this.$el.find('input:checkbox').on('click', function(){
				if(this.checked==true){
					linkedAccounts.push($(this).attr('data-id'));
					this.$el=$('.row').find('tr');
					$(this).closest('.account').addClass('success');

				}
				else{
					for(var idx=0; idx<linkedAccounts.length; idx++){
						if($(this).attr('data-id')==linkedAccounts[idx]){
							linkedAccounts.splice(idx, 1);
							$(this).closest('.account').removeClass('success');
		...
		onClickAddAccount : function(e){
			...
			for(var idx=0; idx<linkedAccounts.length; idx++){
				if(selectedAccountId!=linkedAccounts[idx]){
					if((idx+1) ==linkedAccounts.length){
						linkedAccounts.push(selectedAccountId);
					}
				}
				else{
